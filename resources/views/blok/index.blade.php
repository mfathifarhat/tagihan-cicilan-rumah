    ...
    <div class="card rounded-4 overflow-hidden">
        <div class="card-header bg-white border-0 p-3 d-flex align-items-center justify-content-between">
            <div class="search">
                ...
                <input type="text" class="form-control form-control-solid" placeholder="Search Rumah">
            </div>
            <div class="tools d-flex align-items-center gap-3">
                <a href="{{ route('rumah') }}" class="fs-14 btn btn-primary text-nowrap d-flex align-items-center">
                ...
                <a href="{{ route('create-blok') }}" class="fs-14 btn btn-primary text-nowrap d-flex align-items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
        ...
        <div class="card-body">
                ...
                <table class="table table-sm gy-5 align-middle">
                    ...
                    <tbody>
                        @foreach ($bloks as $item)
                            <tr>
                                <td>{{ $loop->iteration + (15 * ($bloks->currentPage() - 1)) }}</td>
                                <td>{{ $item->blok }}</td>
                                <td>{{ $item->alamat }}</td>
                                <td>
                                    <div class="d-flex align-items-center justify-content-end gap-2">
                                        <a href="{{route('edit-blok', $item->id)}}" class="btn btn-sm no-border ">
                                        ...
                                        <a href="{{route('delete-blok', $item->id)}}" class="btn btn-sm text-danger no-border ">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="20"
            ...
            <div class="pagination row gy-2 justify-content-between align-items-center">
                <div class="col-sm-6 text-center text-sm-start">
                    Page {{$bloks->currentPage()}} of {{$bloks->lastPage()}}
                </div>
                <div class="col-sm-6 text-center text-sm-end">
                    <a href="{{ $bloks->previousPageUrl() }}" class="btn btn-sm btn-primary">Previous</a>
                    <a href="{{ $bloks->nextPageUrl() }}" class="btn btn-sm btn-primary">Next</a>
                </div>
            </div>
        </div>
    </div>
